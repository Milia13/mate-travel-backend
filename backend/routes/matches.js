...
// ─── GET /api/matches/:userId/mensajes ──────────────────────────
// Obtiene el historial de mensajes entre el usuario logueado y userId
router.get("/:userId/mensajes", verificarToken, async (req, res) => {
  try {
    const yo = req.usuario._id.toString();
    const otroId = req.params.userId;
    ...
    const mensajes = await Mensaje.find({
      $or: [
        { emisor: yo, receptor: otroId },
        { emisor: otroId, receptor: yo }
      ]
    }).sort({ fecha: 1 }); // Orden cronológico (antiguos primero)
